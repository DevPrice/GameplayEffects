...
bool ModifierAggregator::get_modified_value(const Ref<GameplayStat>& stat, float base_value, float& modified_value) const {
    ...
    float aggregate_offset = std::accumulate(offset_modifiers.begin(), offset_modifiers.end(), 0.f, [](const float acc, const std::shared_ptr<IEvaluatedModifier> modifier) {
    ...
    float aggregate_multiply = 1.f + std::accumulate(multiply_modifiers.begin(), multiply_modifiers.end(), 0.f, [](const float acc, const std::shared_ptr<IEvaluatedModifier> modifier) {
