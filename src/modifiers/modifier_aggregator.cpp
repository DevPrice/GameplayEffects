...
bool ModifierAggregator::get_modified_value(const Ref<GameplayStat>& stat, float base_value, float& modified_value) const {
    std::vector<IEvaluatedModifier*> relevant_modifiers;
    std::copy_if(modifiers.begin(), modifiers.end(), std::back_inserter(relevant_modifiers), [&stat](const IEvaluatedModifier* modifier) {
    ...
    for (int i = relevant_modifiers.size() - 1; i >= 0; i--) {
        IEvaluatedModifier* modifier = relevant_modifiers[i];
    ...
    std::vector<IEvaluatedModifier*> offset_modifiers;
    std::copy_if(relevant_modifiers.begin(), relevant_modifiers.end(), std::back_inserter(offset_modifiers), [](const IEvaluatedModifier* modifier) {
        return modifier && modifier->get_operation() == StatModifier::Operation::Offset;
    });
    float aggregate_offset = std::reduce(offset_modifiers.begin(), offset_modifiers.end(), 0.f, [](const float acc, const IEvaluatedModifier* modifier) {
        return acc + modifier->get_magnitude();
    });

    std::vector<IEvaluatedModifier*> multiply_modifiers;
    std::copy_if(relevant_modifiers.begin(), relevant_modifiers.end(), std::back_inserter(multiply_modifiers), [](const IEvaluatedModifier* modifier) {
        return modifier && modifier->get_operation() == StatModifier::Operation::Multiply;
    });
    float aggregate_multiply = 1.f + std::reduce(offset_modifiers.begin(), offset_modifiers.end(), 0.f, [](const float acc, const IEvaluatedModifier* modifier) {
