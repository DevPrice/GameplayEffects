...
bool ModifierAggregator::get_modified_value(const Ref<ModifierChannel>& channel, const Ref<GameplayStat>& stat, stat_value_t base_value, stat_value_t& out_modified_value) const {
    ...
    stat_value_t aggregate_offset = std::accumulate(offset_modifiers.begin(), offset_modifiers.end(), STAT_ZERO, [](const stat_value_t acc, const std::shared_ptr<EvaluatedModifier> modifier) {
    ...
    stat_value_t aggregate_multiply = 1.f + std::accumulate(multiply_modifiers.begin(), multiply_modifiers.end(), STAT_ZERO, [](const stat_value_t acc, const std::shared_ptr<EvaluatedModifier> modifier) {
        return acc + modifier->get_magnitude() - 1.f;
    });

    out_modified_value = (base_value + aggregate_offset) * aggregate_multiply;

    return aggregate_offset != STAT_ZERO || aggregate_multiply != 1.f;
}
